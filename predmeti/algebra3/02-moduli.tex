...
\begin{definicija}
  ...
  Potem je
  \[
	IM = \left\{ \sum_{i=1}^k u_i m_i \such u_i \in I, m_i \in M, k \in \N \right\}
  \]
  podmodul v $M$.
\end{definicija}
...
\begin{proof}
  ...
  \otimes_R N \to P$, da naslednji diagram komutira:
  \[\begin{tikzcd}
	  {M \times N} & P \\
	  {M \otimes_R N}
...
\otimes_R M$ $R$-modul z množenjem
\[
...
\begin{trditev}
  Naj bo $0 \to A \xrightarrow{f} B \xrightarrow{g} C \to 0$ kratko eksaktno
  zaporedje $R$-modulov.
  Naslednje trditve so ekvivalentne:
  \begin{itemize}
  \item Obstaja homomorfizem $p: B \to A$, da je $p \circ f = \id_A$
  \item Obstaja homomorfizem $s: C \to B$, da velja $g \circ s = \id_C$
  \item Obstajata homomorfizma $p: B \to A$ in $s: C \to B$, da velja $p \circ f
	= \id_A$, $g \circ s = \id_C$ in $f\circ p + s \circ g = \id_B$.
  \end{itemize}
  V vseh treh primerih je $B \cong A \oplus C$.
\end{trditev}

\begin{proof}
  Dokažimo le implikacijo iz prve točke v drugo; ostalo gre podobno.
  Najprej dokažimo, da je $B = \jedro p \oplus \im f$.
  Naj bo $x$ v preseku.
  Potem je $p(x) = 0$ in $x = f(a)$, torej $a = p(f(a)) = 0$ in $x = 0$.
  Vzemimo $b \in B$.
  Potem je
  \[
	b = f(p(b)) + (b - f(p(b))),
  ...
  Velja $\im f \cong A$.
  Trdimo, da je $\jedro p \cong C$.
  Dokažimo, da je $g$, skrčen na $\jedro p$, izomorfizem.
  Naj bo $c \in C$.
  Ker je $g$ surjektiven, obstaja $b \in B$, da je $g(b) = c$.
  Po prejšnjem je $b = x + y$ za $x \in \jedro p$ in $y \in \im f$, torej je $c
  = g(b) = g(x)$, ker je $\im f = \jedro g$.
  Za injektivnost vzemimo $z \in \jedro p$, za katerega je $g(z) = 0$.
  Ker je $\jedro g = \im f$, je $z = f(y)$ za nek $y \in A$.
  Potem je $0 = p(z) = p(f(y)) = y$, torej $z = 0$.
  Za $s$ lahko torej vzamemo inverz skrčitve $g$ na $\jedro p$.
\end{proof}

\vprasanje{Dokaži: če za kratko eksaktno zaporedje obstaja homomorfizem $p: B
  \to A$, za katerega je $p \circ f = \id_A$, obstaja tudi homomorfizem $s: C
  \to B$, da je $g \circ s = \id_C$.}

\begin{izrek}[kratka lema o petih]
  Imejmo diagram $R$-modulov in homomorfizmov
  \[\begin{tikzcd}
	  0 & A & B & C & 0 \\
	  0 & A' & B' & C' & 0
	  \arrow[from=1-1, to=1-2]
	...
	\end{tikzcd}\]
  pri čemer sta obe vrstici eksaktni in oba kvadrata komutativna diagrama.
  Če sta $\alpha$ in $\gamma$ injektivna, je tudi $\beta$ injektiven.
  Če sta $\alpha$ in $\gamma$ surjektivna, je tudi $\beta$ surjektiven.
\end{izrek}

\begin{proof}
  Dokažemo samo za injektivnost.
  Vzemimo $b \in \jedro \beta$.
  Velja $\gamma(g(b)) = g'(\beta(b)) = g'(0) = 0$, torej je $g(b) = 0$ in $b \in
  \jedro g = \im f$.
  Torej je $b = f(a)$.
  Velja $0 = \beta(f(a)) = f'(\alpha(a))$, torej $\alpha(a) \in \jedro f' =
  \{0\}$ in $a = 0$.
  Torej $b = 0$.
\end{proof}

\vprasanje{Povej in dokaži kratko lemo o petih.}
