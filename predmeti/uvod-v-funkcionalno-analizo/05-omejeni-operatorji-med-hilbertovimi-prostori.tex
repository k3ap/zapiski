...
\vprasanje{Pokaži, da je $K(X,Y)$ zaprt podprostor $B(X,Y)$.}
...
\vprasanje{Pokaži: če za omejeno zaporedje $(f_n)_n$ zveznih funkcij za poljuben
  $\varepsilon > 0$ obstaja $\delta > 0$, da za vsak $n$ iz $\abs{x-y} < \delta$
  sledi $\abs{f_n(x) - f_n(y)} < \varepsilon$, potem ima zaporedje stekališče.}
